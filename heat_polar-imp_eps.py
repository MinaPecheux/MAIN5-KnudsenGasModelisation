...
#For the ring 0 (central point + eps)
    eps=0.1
    dr0=eps
    Ai0   = lambda i: 1 + 2*t/(dr0**2) + 2*t/((i*dr0)**2 * (dtheta**2))
    Bi0   = lambda i: -t/((i*dr0)**2 * (dtheta**2))
    Ci_p0 = lambda i: -t/(dr0**2) * (1 + 1/(2*i))
    Ci_m0 = lambda i: -t/(dr0**2) * (1 - 1/(2*i))


#For the first ring ( P=1 )
    dr1=dr-eps
    Ai1   = lambda i: 1 + 2*t/(dr1**2) + 2*t/((i*dr1)**2 * (dtheta**2))
    Bi1   = lambda i: -t/((i*dr1)**2 * (dtheta**2))
    Ci_p1 = lambda i: -t/(dr1**2) * (1 + 1/(2*i))
    Ci_m1 = lambda i: -t/(dr1**2) * (1 - 1/(2*i))
    
    M = np.zeros((1 + (P-1)*Q, 1 + (P-1)*Q)) # square matrix
    # central point: centered scheme
    # Considerer un amat de points et prendre le milieu 
    M[0,0]      = 1. + 4*t/(dr**2)
    M[0,1]      = -2*t/(dr**2)
    M[0,Q//2+1] = -2*t/(dr**2)
...
#specific traitement for ring 0 ( a ring between the central point and the first ring )

    for q in range(1, Q+1):
        M[q,0]   = Ci_m0(1)
        M[q,q]   = Ai0(1)
        if q == 1: M[q,Q]   = Bi0(1)   # modulo on q indices
        else:      M[q,q-1] = Bi0(1)
        if q == Q: M[q,1]   = Bi0(1)
        else:      M[q,q+1] = Bi0(1)
        M[q,q+Q] = Ci_p0(1)
...
#plt.colorbar(p012, cax=cbaxes)
plt.colorbar(p012, orientation="horizontal")
